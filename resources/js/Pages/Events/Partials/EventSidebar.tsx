...
export default function EventSidebar({ categories = [], filters }: Props) {
        ...
        <aside className="w-full max-w-[380px] overflow-hidden rounded-qa bg-white shadow-qa" data-aos="fade-up">
            ...
            <div className="mt-2 p-[18px]">
                <form onSubmit={handleFilter}>
                    ...
                    {/* Date Picker */}
                    <div className="mb-6">
                        <label className="mb-2 block text-[13px] font-semibold text-black/60 uppercase tracking-wider">
                            Event Date
                        </label>
                        <input
                            type="date"
                            ...
                            className="h-10 w-full rounded-qa border-[#999] text-sm focus:border-primary focus:ring-1 focus:ring-primary"
                        />
                    </div>

                    {/* Category Radios */}
                    <div className="space-y-3">
                        <div className="mb-2 block text-[13px] font-semibold text-black/60 uppercase tracking-wider">
                            Categories
                        </div>
                        <div className="flex items-center gap-2">
                            <input
                                className="border-gray-600 text-primary h-4 w-4 focus:ring-primary cursor-pointer"
                                type="radio"
                                name="eventCategory"
                                value=""
                                ...
                                id="eventcat-all"
                            />
                            <label className="text-[15px] font-light text-black cursor-pointer" htmlFor="eventcat-all">
                                All Events
                            </label>
                        </div>
                        ...
                        {categories.map((cat) => (
                            <div className="flex items-center gap-2" key={cat.id}>
                                <input
                                    ...
                                    id={`eventcat-${cat.id}`}
                                />
                                <label className="text-[15px] font-light text-black cursor-pointer" htmlFor={`eventcat-${cat.id}`}>
                                    {cat.title}
                                </label>
                            </div>
                        ))}
